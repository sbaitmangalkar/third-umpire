...
public class PersonFinder {
     ...
     * @return
     */
    public static List<Voter> findPerson(String firstName, String middleName, String lastName,
        String districtName) {
        List<String> allPollingStationsInDistrict =
            AssemblyConstituencyFinder.findAllAssemblyConstituencies(districtName).entrySet()
                .parallelStream().map(entry -> entry.getValue())
                .map(pl -> PollingStationFinder.findAllPollingStationsURL(pl)).flatMap(List::stream)
                .collect(Collectors.toList());

        System.out.println(
            "Total no. of polling stations in " + districtName + ": " + allPollingStationsInDistrict
                .size());
        ...
        List<SearchResult> searchResults = allPollingStationsInDistrict.parallelStream()
            ...
            .filter(res -> res.getResult().contains(firstName)).collect(Collectors.toList());
     ...
     */
    public static List<Voter> findPerson(String firstName, String middleName, String lastName,
        String districtName, String assemblyConstituencyName) {
        String assemblyConstituencyURL = AssemblyConstituencyFinder
            .getURLForAssemblyConstituency(districtName, assemblyConstituencyName);
        List<String> allPollingStationURLs =
            PollingStationFinder.findAllPollingStationsURL(assemblyConstituencyURL);
     ...
     * @return
     */
    private static List<Voter> processSearchResults(List<SearchResult> searchResults,
        ...
        if (searchResults.size() > 1) {
            voters = searchResults.stream().map(res -> buildVoter(res, firstName, lastName))
                .collect(Collectors.toList());
        } else {
